	...
	\begin{frame}[plain]
		...
		\begin{tikzpicture}
			\begin{semilogyaxis}[
				...
				\addplot+[mark options={color=black,fill=red,fill opacity=.5}] table [only marks,col sep=comma,x=release_date,y=time_in_ms] {\products};
				\addplot[black] table [mark=none,col sep=comma,x=release_date,y={create col/linear regression={x=JulianDayMod,y=time_in_ms}}] {\products};
				\node [coordinate,pin=above:{v2.6.12}] at (axis cs:2005-06-18,334) {};
				\node [coordinate,pin=above:{v3.0}] at (axis cs:2011-07-21,13773) {};
				\node [coordinate,pin=above left:{v4.0}] at (axis cs:2015-04-12,1855100) {};
				\node [coordinate,pin=above:{v4.8 (46 min)}] at (axis cs:2016-09-25,2740432) {};
				\node [coordinate,pin=right:{timeout}] at (axis cs:2018-08-12,3600000) {};
			\end{semilogyaxis}
		\end{tikzpicture}
	\end{frame}
