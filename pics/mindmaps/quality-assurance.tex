...
\begin{frame}[plain]
	...
	\begin{tikzpicture}
		...
		child { node {constructive} 
			[clockwise from=225]
			child[concept color=blue!20,visible on={<2-5>}] { node {Coding Guidelines \scalebox{.7}{\color{blue}(Lecture 9)}} }
		}
		child[] { node {analytical} 
			[counterclockwise from=240]
			child[concept color=green!20,visible on={<4-6>}] { node {analysis \scalebox{.7}{\color{blue}(Lecture 10)}}
				[counterclockwise from=180]
				child { node {Compilation} }
				child { node {Code Reviews} }
			}
			child[concept color=red!20,visible on={<5,7>}] { node {execution \scalebox{.7}{\color{blue}(Lecture 11)}}
		...
		child { node {organizational} 
			[clockwise from=-45]
			child[concept color=orange!20,visible on={<3-5>}] { node {Software Project \mbox{\hspace{-.5mm}Management} \scalebox{.7}{\color{blue}(Lecture 9)}} }
		}
		;
	\end{tikzpicture}
\end{frame}
