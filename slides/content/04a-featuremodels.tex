...
\begin{frame}{\myframetitle}\label{frame:cfgmap}
	\begin{mycolumns}
		...
		\mydefinition{Configuration Map}{
			\begin{itemize} % TODO symbol C is overloaded on this slide. shall we use M for configuration map?
				\item a \emph{configuration map} over $F$ is a set of complete configurations $C \subseteq F$ % TODO seems to be wrong, this rule holds for every element in a configuration map
...
\begin{frame}{\myframetitle}
	\centering
	\pic[width=0.48\linewidth]{products-in-excel}
...
\begin{frame}{\myframetitle\ \mytitlesource{\fospl; \foda\mypages{63--72}; \batorygrammars}}
	\begin{mycolumns}
		\myexampletight{}{ % TODO do we really need these borders here and on the next slides?
			\centering
...
\begin{frame}{\myframetitle}
	\begin{mycolumns}
		...
		\mynote{Pro: Tool Support}{ % TODO would be great to show more tool logos here from our community, and possibly also for commercial tools (upon request)
