...
\begin{frame}{\myframetitle}
	...
	\begin{mycolumns}[animation=none]
		\mydefinition{Recap: Conjunctive Normal Form}{
			\begin{itemize}
				...
				\item a \emph{conjunctive normal form (CNF)} is a conjunction of clauses $\bigwedge_{i} C_i = \bigwedge_{i} \clause{\bigvee_{j} L_j}$
	...
	\mynextcolumn
		\uncover<2->{
			% \myexampletight{A Feature Model $FM$ \ldots}{
			% 	\centering\tiny
			% 	\featureDiagramConfigurableDatabaseSmall
			% }
			\myexample{Transforming Part of $\phi(FM)$ into CNF}{
				\vspace*{-4ex}
				\small
				% todo: introduce colors that are color-blind-compatible
				\leftandright{%
					\begin{align*}
						\uncover<2->{&~\clause{C} \\
						%\pand &~(A \pequals C) \\
						\pand &~(T \pimplies C) \\
						\pand &~(O \pequals C) \\
						%\pand &~(G \por P \por D \pequals A) \\
						\pand &~(W \por L \pequals O) \\
						\pand &~\pnot (W \pand L)}
						%\pand &~(T \pimplies P \por D) \\
					\end{align*}
				}{
					\begin{align*}
						\uncover<3->{&~\clause{C} \\
...
\begin{frame}{\myframetitle}
	\myexampletight{Representations of Variability Models}{
		\includegraphics[width=\linewidth]{representations-low-level}%also kconfig?
