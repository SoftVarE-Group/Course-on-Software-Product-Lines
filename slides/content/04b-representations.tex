\subsection{Natural Language, Configuration Map, and Feature Model}

\subsubsection*{Variability Model Representations}
	...
	\mynextcolumn
		...
		\mynote{Problems}{
			\begin{enumerate}
				...
				\item<4->[P2] How to (a) validate configurations and (b) obtain valid configurations \emph{automatically}?
	...
	\mynextcolumn
		\mynote{Problems}{
			\begin{enumerate}
				...
				\item[P2] How to (a) validate configurations and (b) obtain valid configurations \emph{automatically}?
	...
	\begin{mycolumns}[animation=none]
		\mydefinition{Algorithm: Translate $FM$ Into $\Phi(FM)$}{
			\begin{enumerate}
				...
				\item<9-> conjoin cross-tree constraints\\
...
\begin{frame}
	\begin{mycolumns}[widths={52,48}]
		...
		\begin{tikzpicture}
			...
			\path [dotted, ->] (cfg) edge[bend right=15] node[sloped,yshift=1mm] {CNF} (cnf);
		...
		\mynote{Problems}{
			\begin{enumerate}
				...
				\begin{itemize}
					...
					\item[(b)] obtain valid configurations
				\end{itemize}
				\emph{automatically}?
				\item[P3] \color{gray}{(How to reverse engineer feature models?)}
			\end{enumerate}
		...
		\mynote{Solutions}{
			\begin{enumerate}
				...
				\begin{itemize}
					...
					\item[(b)] \emph{???} $\Rightarrow$ soon \ldots
				\end{itemize}
...
\subsection{Variability Models in the Wild}

\subsubsection*{Excel, Again}

\begin{frame}{\myframetitle}
	\centering
	\pic[width=.7\linewidth,trim=10 10 0 10,clip]{constraints-in-excel}
...
\begin{frame}[fragile]{\myframetitle}
	\texttt{\href{https://github.com/torvalds/linux/blob/0326074/arch/x86/Kconfig}{torvalds/linux/arch/x86/Kconfig}}
	\begin{mycolumns}
		\begin{kconfig}[basicstyle=\small]
config 64BIT
	...
	\mynextcolumn
		\mydefinition{KConfig Language\mysource{\href{https://www.kernel.org/doc/html/latest/kbuild/kconfig-language.html}{kernel.org}}}{
			\begin{itemize}
				...
				\item translation into formula or feature model possible, but not trivial \mysource{\href{https://dl.acm.org/doi/abs/10.1145/3468264.3468578}{Oh~et~al.~2021}}
			\end{itemize}
		}
	\hspace*{-3mm}\href{https://dl.acm.org/doi/abs/10.1145/3382025.3414943}{\includegraphics[width=1.2\linewidth,trim=180 510 100 170,clip]{2020/2020-SPLC-Thuem}}
	\end{mycolumns}
\end{frame}