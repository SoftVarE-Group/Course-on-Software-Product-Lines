...
% TODO excurs on selectors? amazon-tshirt, ebay-smartphone-case

% TODO unclear why certain options are missing: windows10-sign-in-options

\subsubsection*{Cars}

\begin{frame}{\myframetitle}
	\begin{mycolumns}[b,widths={67}]
		\myexampletight{Configuring a car \ldots}{
			\centering\small
			...
			\only<14|handout:0>{\pic[width=\linewidth]{toyota-aygo-wheels3}}%
			\only<15-|handout:0>{\pic[width=\linewidth]{toyota-aygo-wheels4}}%
		}
	\mynextcolumn
		\vspace{-\textheightoftitle}
		\myexample{\ldots\ is complicated!}{
			\begin{enumerate}
			...
			\end{enumerate}
			what now? back to Step 3?
		}
	\end{mycolumns}
\end{frame}

\begin{frame}{\myframetitle}
	\begin{mycolumns}[t,columns=3,widths={33,22}]
		\myexampletight{Configuring a Car \ldots}{
			\pic[width=\linewidth]{toyota-aygo-wheels3}\\\centering
			10. cancel the dialog
		}
	\mynextcolumn
		\myexampletight{with a Weird Price}{
			\centering\pic[width=\linewidth]{toyota-aygo-costs}
		}
	\mynextcolumn
		\myexampletight{with 8 Wheels!}{
			\includegraphics[width=\linewidth]{toyota-aygo-costs3}
		}
	\end{mycolumns}
	\begin{mycolumns}[columns=3,widths={20,60},animation=none]
	\mynextcolumn
		\uncover<4->{\mynote{}{
			\begin{itemize}
...
\begin{frame}{\myframetitle}
	\begin{mycolumns}[widths={70,30}]
		\myexampletight{Configuring a German Car\mysource{example from \lectureintroduction}}{
...
\begin{frame}{\myframetitle}
	\begin{mycolumns}[widths={65}]
		\myexampletight{Configuring a Notebook}{ % it is on purpose that both pictures are shown in handout
			\only<1>{\pic[width=\linewidth]{thinkpad-x1-yoga-display}}
			\only<2->{\pic[width=\linewidth]{thinkpad-x1-yoga-display-invalidconf}}
		}
	\mynextcolumn
		\mynote{}{
...
\begin{frame}{\myframetitle}
	...
	\mynextcolumn
		...
		\uncover<3->{\mydefinition{Solution Enumeration Problem (AllSAT)}{
			\begin{itemize}
...
\begin{frame}{\myframetitle}
	...
	\mynextcolumn
		...
		\myexampletight{Time to Count Products of Linux}{
			\includegraphics[width=\linewidth,page=4]{linux-plots}\\[-7mm]
			\begin{itemize}
...
\begin{frame}{\myframetitle}
	\begin{mycolumns}
		...
		} % TODO better example? what about the number of features such that it cannot be stored on all servers of the world? or we take a reasonable size of a hard disk in a notebook? 5 terrabyte? I (Thomas) cannot imagine a petabyte.
		\myexampletight{}{
			\leftandright{
				\centering
				{\small\featureDiagram{Root,concrete[X,concrete,alternative][Y,concrete]}\\$\pnot (X \por Y)$}
...
\begin{frame}{\myframetitle}
	\begin{mycolumns}[widths={60,40}]
		\myexampletight{}{ % TODO would be great to show an explanation here
