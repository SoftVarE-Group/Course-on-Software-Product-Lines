...
\begin{frame}{Traditional Promises of Microservices}
	\begin{mycolumns}[widths={50,50},animation=none]
		\mynote{}{
			\begin{itemize}
				\item Scalability: Microservices are small enough to be developed by a small, agile team (cf. typical size of a Scrum team).
				\item Continuous integration/deployment: Microservices can be deployed independently of each other (i.e., each microservice has its own CI/CD pipeline).
				\item Heterogeneity: Each microservice can be implemented using its own technology stack (team preferences, adoption of new technologies, etc.).
				\item Fault tolerance: The crash of a single microservice should not lead to a crash of the entire system (cf. design for failure).
				\item Efficiency: Configuration of execution environment can be optimized per microservice.
				\item Modernization: A microservice can be easily replaced by an alterantive one (even being re-implemented from scratch).
			\end{itemize}
		}
	\mynextcolumn
		...
		\myexample{Recap: Component-Based Implementation}{
			\includegraphics[width=.23\linewidth,height=1.0cm]{lego_components} 
				\vspace*{\fill}
					$+$ 
				\vspace*{\fill}	
			\includegraphics[width=.23\linewidth,height=1.0cm]{lego_glue}
				\vspace*{\fill}
					$=$ 
				\vspace*{\fill}	
