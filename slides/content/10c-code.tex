...
\begin{frame}{\myframetitle}
	\begin{mycolumns}[widths={35,65},animation=none]
		\mydefinition{Reachability Condition of $id$}{
			guarantees that each reference to $id$ is \emph{resolvable}:\\
			$\Phi(FM) \mimplies (PC_{ref}^{id} \pimplies PC_{def}^{id})$\\[1ex]
			or, with a SAT solver:\\
			$\pnot SAT(\Phi(FM) \pand PC_{ref} \pand \pnot PC_{def})$
		}
		\mydefinition{Type-Safe Product-Line}{
			in a \emph{type-safe} SPL, all references must be reachable (i.e., all reachability conditions hold)\\
			{\color{gray}\ldots\ and many more conditions \ldots}
		}
		\myexample{}{
			$\Phi(FM) \mimplies (SS \por MS)$ holds,\\
...
% \mydefinition{Conflict Condition of $id$}{
% 	guarantees that $id$ is \emph{defined once}:\\
% 	$\Phi(FM) \mimplies \pnot (PC_{def_1}^{id} \pand PC_{def_2}^{id})$\\[1ex]
% 	or, with a SAT solver:\\
% 	$\pnot SAT(\Phi(FM) \pand PC_{src} \pand \pnot PC_{dst})$
% }
% \myexample{}{
% 	$VM \mimplies (AccessControl \pimplies MultiStore)$
% }
...
\begin{frame}{\myframetitle}
	...
	\mynextcolumn
		\mynote{}{
			here: \emph{family-based} analysis strategies for \emph{conditional compilation/FOP}
		}
	\end{mycolumns}
\end{frame}
