...
\begin{frame}{Pairwise Coverage \deutschertitel{Paarweise Überdeckung}}
	\begin{mycolumns}[animation=none]
		\centering
		\only<1>{\featureDiagramConfigurableDatabase}%
		\only<2>{\featureDiagramConfigurableDatabaseNoAbstract}%
		\only<3->{\featureDiagramConfigurableDatabaseNoAbstractNoCore}
...
\begin{frame}{\myframetitle{} \deutschertitel{T-Weises Interaktionstesten}}
