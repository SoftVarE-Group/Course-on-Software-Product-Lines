...
\begin{frame}{\insertsubsection\ \mytitlesource{\fospl}}\label{frame:fm}
	\leftandright{
		\myexampletight{A Configurable Database}{
			...
			\only<1-3>{\emphfeatures{}}
			\only<4>{\emphfeatures{ConfigDB,Get,Windows}}
			\only<5>{\emphfeatures{ConfigDB,Get,Put,Delete,Transactions,Linux}}
			\only<6>{\emphfeatures{ConfigDB,Get}}
			\only<7>{\emphfeatures{ConfigDB,Get,Transactions,Linux}}
